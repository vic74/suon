...
uses
  ...
  dxScreenTip, dxSkinDevExpressStyle, Vcl.ComCtrls, dxCore, cxDateUtils,
  cxNavigator, KontragentRepository;

type
  TTRProcess6Form = class(TDezForm)
    grp1: TdxLayoutGroup;
...
var
  TRProcess6Form: TTRProcess6Form;
...
procedure TTRProcess6Form.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  fController.Free;
  Action:=caFree;
end;

procedure TTRProcess6Form.FormCreate(Sender: TObject);
begin
...
procedure TTRProcess6Form.PrepareDS;
begin
  CreateMState;
  {$REGION 'Kontragent'}
...
procedure TTRProcess6Form.PrintRep;
var
  Q1: TfrxFIBQuery;
  fdb: TfrxFIBDatabase;
begin
...
procedure TTRProcess6Form.ReopenTRFACT;
begin
...
procedure TTRProcess6Form.ReopenTRPREDP;
begin

  with ds_TRPREDP do begin
    Close;
      if gFN(v1,'CODE') = null then exit;
      MainWhereClause:='tp.TRFACT = :tf';
      ParamByName('tf').AsVariant:=gFN(v1,'CODE');
    Open;
  end;
end;

procedure TTRProcess6Form.SetRight;
begin
  case ActiveRole of
...
procedure TTRProcess6Form.v1CellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
  ReopenTRPREDP;
...
procedure TTRProcess6Form.v1CustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TTRProcess6Form.v1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
    VK_UP : ReopenTRPREDP;
...
procedure TTRProcess6Form.AddNewWork;
var
  F :TFactAddForm;
begin
  //if gFN(v1,'CODE')=null then Exit;
...
procedure TTRProcess6Form.btn_AddNewWorkClick(Sender: TObject);
begin
  if ds_TRFACT.State in [dsInactive] then Exit;
  AddNewWork;
end;

procedure TTRProcess6Form.btn_del_factClick(Sender: TObject);
begin
  Del_Plan;
end;

procedure TTRProcess6Form.btn_execClick(Sender: TObject);
begin
  ReopenTRFACT;
end;

procedure TTRProcess6Form.btn_printClick(Sender: TObject);
begin
  PrintRep;
end;

function TTRProcess6Form.CheckParams: Boolean;
...
function TTRProcess6Form.CheckTRdopState: Boolean;
var
  TRDOPSOG :Variant;
  Q :TFIBQuery;
begin
...
procedure TTRProcess6Form.clv1DATACustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TTRProcess6Form.clv1DATAPropertiesEditValueChanged(Sender: TObject);
begin
...
procedure TTRProcess6Form.clv1DPLACEPropertiesEditValueChanged(Sender: TObject);
begin
  v1.DataController.Post();
end;

procedure TTRProcess6Form.clv1NOTEPropertiesEditValueChanged(Sender: TObject);
begin
  v1.DataController.Post();
end;

procedure TTRProcess6Form.clv1STATECustomDrawCell(
...
procedure TTRProcess6Form.clv1STATEPropertiesEditValueChanged(Sender: TObject);
begin
  if gFN(v1,'TRDOPSTATE')=1 then begin
     Application.MessageBox('Дополнительное соглашение заблокировано для редактирования.',
       'Ошибка', MB_OK + MB_ICONSTOP + MB_DEFBUTTON2 + MB_TOPMOST);
...
procedure TTRProcess6Form.clv1SUMMACustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
begin
...
procedure TTRProcess6Form.clv1SUMMAPropertiesEditValueChanged(Sender: TObject);
begin
  if gFN(v1,'TRDOPSTATE')=1 then begin
     Application.MessageBox('Дополнительное соглашение заблокировано для редактирования.',
       'Ошибка', MB_OK + MB_ICONSTOP + MB_DEFBUTTON2 + MB_TOPMOST);
...
procedure TTRProcess6Form.CreateMState;
...
procedure TTRProcess6Form.Del_Plan;
begin
  if gFN(v1,'CODE') = null then Exit;
