             ...
             Begin
                ...
                if (Item.CompressionMethod=8) Then
                   Begin
                      ...
                      Try
//  showmessage('check 4');
//  showmessage(inttostr(FParent.FCurrentDFS));
//  showmessage('BUF= '+inttostr(length(BUF)));
//  showmessage('size= '+inttostr(sfs.Size));
                        // crash is here
     ...
     Begin
       ...
       if (CompressionMethod=8) Then
          Begin
            ...
            SetLength(Result,SizeOf(FZLHeader));
            // modified D2009
            move(FZLHeader,Result,SizeOf(FZLHeader));
            // SetString(Result,PChar(@FZLHeader),SizeOf(FZLHeader));
          End;
       Result := Result + BA.CompressedData;
     End
...
procedure TKAZipEntries.InternalExtractToFile(Item: TKAZipEntriesEntry; FileName: AnsiString);
Var
  TFS           : TFileStream;
  Attr          : Integer;
begin
...
function TKAZip.AddFilesAndFolders(FileNames: TStrings; RootFolder: AnsiString; WithSubFolders: Boolean): Boolean;
begin
  Result := Entries.AddFilesAndFolders(FileNames,RootFolder,WithSubFolders);
end;
