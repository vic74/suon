...
{for version Delphi 1..7, 2005, 2006/Turbo, 2007 and partial 2009 compatibility}
{history:}
...
{$ELSE}
uses SysUtils, Classes, Dialogs, Windows, RussianCodeTables;
{$ENDIF}

const

  msgCreateDBFError = 'Ошибка при создании нового DBF файла.';
  msgErrorOnOpen = 'Ошибка при попытке открыть DBF файл.';
  msgEOFmarkMiss = 'Не найден конец файла.';
  msgNoPasswordMatch = 'Некорректный пароль.';
  msgNotEnoughtCreateIdx = 'Ошибка при попытке создать IDX файл.';
  msgIdxTableNotFound = 'Не найден файл индекса IDX "%s".';
  msgErrorOnIdxTable = 'Ошибка чтения файла индекса IDX "%s".';
  msgIdxFieldNotFound = 'Файл индекса "%s" не найден.';
  msgFieldNotFound = 'Файл %d не найден.';
  msgFileIsTooLarge = 'Файл слишком большой.';
  msgErrorOnWrite = 'Ошибка при записи данных.';
  msgFileTooRemote = 'File too remote.';
  msgCannotOpenTable = 'Ошибка при открытии DBF файла.';
  msgCannotDeleteItem = 'Ошибка при удалении записи таблицы.';
  msgCannotAddItem = 'Ошибка при вставке записи в таблицу.';
  msgBadDefinitionOfField = 'Некорректное описание поля DBF файла.';
  msgDuplicateInUnique = 'Дублирование значения уникального индекса.';
  msgErrorOnMemoOpen = 'Ошибка чтения мемо поля таблицы.';
  prgMakeIndexSort = 'Сортировака индекса...';
  prgWriteIndexSort = 'Сохранение отсортированного индекса...';
  prgSearchByKey = 'Поиск записи...';
...
{$ENDIF}
  ...
  SpacerD = ' ';
type
  charay11 = array[0..10] of AnsiChar; {filename defs.}
  ...
  TDBFError = procedure(Sender: TObject; const ErrorMsg: AnsiString) of object;
  TDBFMakeItems = procedure(Sender: TObject; Posit: Integer;
    var INname: ChAray11; var IWhat: AnsiChar; var ILen, IPlaces: Byte;
    var IDXName: charay11; var Req: TFieldReq; var Sort: TSortByIndex) of object;
  TDBFProgress = procedure(Sender: TObject; const Operace: AnsiString; Progress: Integer) of object;
  TDBFPassword = procedure(Sender: TObject; var Pass: AnsiString) of object;
  TDBFAssigned = procedure(Sender: TObject; var FName: String) of object;
  TDBFBeforeConfirm = procedure(Sender: TObject; const FName: AnsiString; var Confirm: Boolean) of object;
  ...
  TDBFQuery = procedure(Sender: TObject; const IdxName, IdxField, Key: AnsiString;
...
const
  TdbTypes: set of AnsiChar = ['C' {characters, all ascii},
...
{----------------------------------------------------------------------------}
  ...
  private
    ...
    FDBFIndex: AnsiString; { Actual index for FIND }
    FDBFPassword: AnsiString; { Administrator password }
    FDBFFilter: AnsiString; { not used}
    ...
    procedure SetPassword(const thepassword: AnsiString);
  ...
  public
    ...
    procedure MakeIndex(const IdxName: AnsiString; const Key: TKey); {make index} virtual;
    procedure Find(const IdxName, Value: AnsiString); {search value by key} virtual;
    procedure Store(const Key: TKey; const Rec: AnsiString); {field of record} virtual;
    procedure ELoad(const Key: TKey; var Rec: AnsiString); {with conversion} virtual;
    function Load(const Key: TKey): AnsiString;
    procedure Update(R: LongInt); virtual;
    procedure CreateDB(const fname: AnsiString; rL {reclen}, numFields: word); virtual;
    function MakeField(posit: Byte; const iname: AnsiString; iwhat: AnsiChar; ilen: byte;
    ...
    procedure RemoveIndex(const Name: AnsiString); virtual;
    function IsMarked: Boolean; {is in transaction?} virtual;
    function ReIndex: Boolean; virtual;
    procedure IncNumRec;
    function SaveMemo(No: LongInt; const FName: AnsiString): Boolean; virtual;
    function LoadMemo(No: LongInt; var FName: AnsiString): Boolean; virtual;
  ...
  published
    ...
    property ByIndex: AnsiString read FDBFIndex write FDBFIndex;
    property Password: AnsiString read GetPassword write SetPassword;
    property DOSCodes: boolean read fDOSCodes write fDOSCodes;
...
{----------------------------------------------------------------------------}
begin
  RegisterComponents('CoolMan', [TjbDBF]);
end;
...
function TjbDBF.GetPassword: AnsiString;
{----------------------------------------------------------------------------}
{----------------------------------------------------------------------------}
begin
...
procedure TjbDBF.SetPassword(const thepassword: AnsiString);
{----------------------------------------------------------------------------}
{----------------------------------------------------------------------------}
begin
...
procedure TjbDBF.CreateDB(const fname: AnsiString; rL {reclen}, numFields: word);
{----------------------------------------------------------------------------}
{----------------------------------------------------------------------------}
var
  y, m, d: Word;
...
function TjbDBF.MakeField(posit: Byte;
  const iname: AnsiString;
  iwhat: AnsiChar;
  ilen: byte;
  iplaces: byte;
  const idxnme: AnsiString; {filename xxxxxxxxIDX}
  Req: TFieldReq;
  Sort: TSortByIndex
  ): Boolean;
{----------------------------------------------------------------------------}
...
procedure TjbDBF.Fatal(const Msg: AnsiString);
{----------------------------------------------------------------------------}
{----------------------------------------------------------------------------}
begin
...
procedure TjbDBF.Warn(const Msg: AnsiString);
{----------------------------------------------------------------------------}
{----------------------------------------------------------------------------}
begin
...
procedure TjbDBF.MakeIndex(const IdxName: AnsiString; const Key: TKey);
{----------------------------------------------------------------------------}
{vytvori index}
{Make the index}
{----------------------------------------------------------------------------}
...
procedure TjbDBF.Store(const Key: TKey; const Rec: AnsiString);
{----------------------------------------------------------------------------}
...
begin
  ...
  //авто перекодировка строк
  if DOSCodes then
    S:=WinToDOS(S);
  Move(S[1], FDBFBuff^[Posic], FDBFFields[I].Len);
end;

procedure TjbDBF.ELoad(const Key: TKey; var Rec: AnsiString);
{----------------------------------------------------------------------------}
...
function TjbDBF.Load(const Key: TKey): AnsiString;
{----------------------------------------------------------------------------}
...
procedure TjbDBF.Find(const IdxName, Value {toto je vlastne klic}: AnsiString);
{----------------------------------------------------------------------------}
...
{----------------------------------------------------------------------------}
  function SizeOfKey(const Key: AnsiString): Integer;
...
procedure TjbDBF.RemoveIndex(const Name: AnsiString);
{----------------------------------------------------------------------------}
...
begin
  ...
  try
    ...
    try
      ...
      B := 0;
      BlockWrite(F, B, 1);
      SysUtils.DeleteFile(ChangeFileExt(Name, '.mdx'));
    finally
      CloseFile(F);
    end;
  except
...
function TjbDBF.SaveMemo(No: LongInt; const FName: AnsiString): Boolean;
{----------------------------------------------------------------------------}
...
function TjbDBF.LoadMemo(No: LongInt; var FName: AnsiString): Boolean;
{----------------------------------------------------------------------------}
...
begin
  ...
  try
    ...
    try
      ...
      {write to disk}
{$IFDEF VER12UP}
      FName := ChangeFileExt(FName, '.' + AnsiString(T.AsFileType));
{$ELSE}
...
begin
 ...
 {end of file; end of comment 18.1.2009 by J.B.}
end.
