...
procedure TRDocForm.btn_addClick(Sender: TObject);
begin
  AddRec();
end;

procedure TRDocForm.btn_delClick(Sender: TObject);
begin
...
procedure TRDocForm.btn_edtClick(Sender: TObject);
begin
  AddRec(true);
end;

procedure TRDocForm.btn_wClick(Sender: TObject);
var
  F :TAddMatForm;
begin
...
procedure TRDocForm.AddRec(isEdit: Boolean);
begin
...
procedure TRDocForm.cbb_srvPropertiesChange(Sender: TObject);
var
...
procedure TRDocForm.cbb_streetPropertiesEditValueChanged(Sender: TObject);
begin
  OpenHouse;
end;

function TRDocForm.CheckParam: Boolean;
...
procedure TRDocForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  fController.Free;
  Action:=caFree;
end;

procedure TRDocForm.FormCreate(Sender: TObject);
begin
...
procedure TRDocForm.grp2Button0Click(Sender: TObject);
begin
  dt1.EditValue:=null;
...
begin
  ...
  //ds_serviswork.Open;
  DSOptoins(ds_tb,'','TRWORKBDR','CODE','TRWORKBDR_GEN');
  ConfigureCombo(cbb_srv);
end;

procedure TRDocForm.OpenHouse;
begin
  with ds_house do
  begin
    Close;
...
procedure TRDocForm.v1CellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
  FillEdt;
...
procedure TRDocForm.v1KeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case key of
    VK_UP :FillEdt;
    VK_DOWN :FillEdt;
