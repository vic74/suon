...
type
  ...
  private
    ...
    function CreateFirebirdAdapterFromIniFile: IDBConnection;
...
implementation
uses
  Variants, DB, SysUtils, TypInfo, cmDBUnit;
...
begin
  ...
  if not FileExists(ConnectionFileName) then
    Exit;
  Result:= CreateFirebirdAdapterFromIniFile;
  Exit;
...
function TDBConnection.CreateFirebirdAdapterFromIniFile: IDBConnection;
begin
  //DataModule1.fDB1.Connected:= True;
  Result := TFIBPlusConnectionAdapter.Create(cmDb.DB, False);
end;
...
begin
  if FConnection <> nil then
    Exit(FConnection);
    FConnection := CreateFirebirdAdapterFromIniFile;
  Result := FConnection;
end;
