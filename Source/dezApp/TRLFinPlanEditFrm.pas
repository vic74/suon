...
uses
  ...
  dxLayoutLookAndFeels, StdCtrls, cxButtons, BaseForm,
  FIBQuery, pFIBCacheQueries, dxLayoutControlAdapters, dxLayoutContainer,
  dxSkinDevExpressStyle;

type
  TTRLFinPlanEditForm = class(TDezModalForm)
...
var
  TRLFinPlanEditForm: TTRLFinPlanEditForm;
...
procedure TTRLFinPlanEditForm.CalcProc;
begin
  Proc4.Value:=100-Proc1.Value-Proc2.Value-Proc3.Value;
end;

procedure TTRLFinPlanEditForm.CalcSum;
var
  C: currency;
begin
  if ced_sumAll.Value=0 then Exit;
  Sum1.Text:=FormatFloat('0.00',ced_sumAll.Value/100*Proc1.Value);
  Sum2.Text:=FormatFloat('0.00',ced_sumAll.Value/100*Proc2.Value);
  Sum3.Text:=FormatFloat('0.00',ced_sumAll.Value/100*Proc3.Value);
  Sum4.Value:=ced_sumAll.Value-Sum1.Value-Sum2.Value-Sum3.Value;
end;

procedure TTRLFinPlanEditForm.cxButton1Click(Sender: TObject);
begin
...
procedure TTRLFinPlanEditForm.cxButton2Click(Sender: TObject);
begin
  ModalResult:=mrCancel;
end;

procedure TTRLFinPlanEditForm.FormShow(Sender: TObject);
begin
  grp_top.Caption:=street+' '+nomer;
  FillParam;
end;

procedure TTRLFinPlanEditForm.Proc1PropertiesChange(Sender: TObject);
begin
  CalcProc;
  CalcSum;
end;

procedure TTRLFinPlanEditForm.Proc2PropertiesChange(Sender: TObject);
begin
  CalcProc;
end;

procedure TTRLFinPlanEditForm.Proc4PropertiesEditValueChanged(Sender: TObject);
begin
  if Proc4.EditValue<0 then
...
procedure TTRLFinPlanEditForm.YSumPropertiesChange(Sender: TObject);
begin
  CalcSum;
end;

procedure TTRLFinPlanEditForm.FillParam;
var
  Q :TFIBQuery;
begin
  Q:=GetQueryForUse(cmDB.RT,'SELECT * from TRLFINPLAN where Y=:y and HOUSE=:h');
  ...
  begin
    ...
    ced_sumAll.EditValue:= Q.FN('SUMY').AsVariant + Q.FN('SUMOST').AsVariant;
...
function TTRLFinPlanEditForm.CheckParam :Boolean;
begin
  Result:=True;
...
procedure TTRLFinPlanEditForm.AddRecord;
var
  Q :TFIBQuery;
  sql :string;
begin
  ...
  CheckTran;
  sql:=
  'UPDATE OR INSERT INTO TRLFINPLAN '+
